...
#  conv layer4
W_conv4 = tf.Variable(tf.truncated_normal(shape = [3, 3, 192, 192], stddev = 5e-2))
...
#  conv layer5
W_conv5 = tf.Variable(tf.truncated_normal(shape = [3, 3, 192, 192], stddev = 5e-2))
b_conv5 = tf.Variable(tf.constant(value = 0., shape = [192]))
h_conv5 = tf.nn.relu(conv2d(h_drop4, W_conv5) + b_conv5)
h_drop5 = tf.nn.dropout(h_conv5, keep_prob)
#  conv layer6
W_conv6 = tf.Variable(tf.truncated_normal(shape = [1, 1, 192, 192], stddev = 5e-2))
b_conv6 = tf.Variable(tf.constant(value = 0., shape = [192]))
h_conv6 = tf.nn.relu(conv2d(h_drop5, W_conv6) + b_conv6)
h_drop6 = tf.nn.dropout(h_conv6, keep_prob)
#  conv layer7
W_conv7 = tf.Variable(tf.truncated_normal(shape = [1, 1, 192, 10], stddev = 5e-2))
b_conv7 = tf.Variable(tf.constant(value = 0., shape = [10]))
h_conv7 = tf.nn.relu(conv2d(h_drop6, W_conv7) + b_conv7)
h_pool7 = tf.nn.avg_pool(h_conv7, ksize = [1, 8, 8, 1], strides = [1, 8, 8, 1], padding = 'SAME')
h_drop7 = tf.nn.dropout(h_pool7, keep_prob)
y_conv = tf.reshape(h_drop7, [-1, 10])
