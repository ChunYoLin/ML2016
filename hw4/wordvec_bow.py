...
for wv in model.vocab:
    if wv not in ignore_word:
        ...
        wd_id[wv] = wid
        wid += 1
word_group = KMeans(n_clusters = wd_class, random_state = 0).fit_predict(wd_vec)
...
title_bow_lsa = TruncatedSVD(n_components = 10, n_iter = 20, random_state = 42).fit_transform(title_bow)
center = []
center_num = 20
center_arg = []
...
for i in range(center_num):
    if i == 0:
        ...
        inner = np.inner(center[i - 1], fea_vec)
        for xid, x in enumerate(np.argsort(np.abs(inner))):
            if x not in center_arg and np.sum(fea_vec[x]) != 0:
                center.append(fea_vec[x])
                center_arg.append(x)
                break
center = np.asarray(center)
Group = KMeans(n_clusters = 20, init = 'k-means++', random_state = 0, max_iter = 1000).fit_predict(fea_vec)
#  Group = KMeans(n_clusters = 20, init = center, random_state = 0, max_iter = 1000).fit_predict(fea_vec)

tag = np.zeros(shape = 20, dtype = np.int32)
for c in Group:
    tag[c] += 1
print tag
with open('./check_index.csv', 'r') as f_in, open('pred_w2v_bow.csv', 'w') as f_out:
    f_out.write('ID,Ans\n')
...
reduced_data = PCA(n_components = 2).fit_transform(fea_vec)
